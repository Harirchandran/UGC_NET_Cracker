...
const advisory=q=>{const st=String(q.transcriptionStatus||'');return st.startsWith('quarantined')||st.includes('vector-primary')||st.includes('vector-options-primary')||Number(q.transcriptionConfidence||1)<.7};
...
const questionDisplay=q=>{const vectors=vectorStack(q);const st=String(q.transcriptionStatus||'');const passage=q.passage?`<div class="passage"><strong>Shared passage / data:</strong><p>${esc(q.passage)}</p></div>`:'';const text=q.question?`<div class="q-text">${esc(q.question)}</div>`:'';const stem=q.stemVectorSvg?`<div class="vector-sheet semantic-visual">${safeVector(q.stemVectorSvg)}</div>`:'';if(st.includes('vector-primary')&&vectors){const searchable=passage||text?`<details class="transcription-details"><summary>Open searchable transcription (advisory)</summary><div class="alert warning small">The exact vector view above is authoritative for symbols, tables and layout. This transcription is provided for search and accessibility review.</div>${passage}${text}</details>`:'';return `${stem}<div class="exact-vector primary-vector"><div class="visual-caption"><strong>Exact offline vector reconstruction</strong><span>Scales without a raster page image</span></div>${vectors}</div>${searchable}`}const exact=vectors?`<details class="vector-details"><summary>Open exact source-vector verification view</summary>${vectors}</details>`:'';return passage+text+stem+exact};
const optionTextValid=o=>o&&!/^(?:Option\s*)?[A-D1-4](?:\s*\(.*vector.*\))?$/i.test(String(o).trim());
const optionDisplay=(q,o,i)=>{const svg=Array.isArray(q.optionVectorSvgs)?safeVector(q.optionVectorSvgs[i]):'';const alt=Array.isArray(q.optionAlt)?q.optionAlt[i]:'';const label=optionTextValid(o)?String(o):(alt||`Option ${String.fromCharCode(65+i)}`);if(svg)return `<span class="option-content"><span class="option-title">${esc(label)}</span><span class="option-vector">${svg}</span></span>`;return `<span class="option-content">${esc(label)}</span>`};
const optionLabel=(q,i)=>{const o=q.options?.[i],alt=Array.isArray(q.optionAlt)?q.optionAlt[i]:'';return optionTextValid(o)?String(o):(alt||`Option ${String.fromCharCode(65+i)}`)};
const verificationLabel=q=>{if(q.dropped)return 'Official source item · dropped';const st=String(q.transcriptionStatus||'');const cv=String(q.contentVerification||'');if(st.includes('semantic-svg'))return 'Manually reconstructed text + semantic SVG';if(cv.includes('manual-source-page'))return 'Manually reconstructed from source page';if(st.includes('vector-primary')||q.hasExactVectorReconstruction||cv.includes('exact-vector'))return 'Exact source-vector preservation';if(cv.includes('clean-native')||st==='verified-text')return 'Native text structurally checked';if(cv.includes('structured')||st.includes('machine-transcribed'))return 'Structured transcription · review metadata retained';return 'Content status not independently established'};
